...
## v5.0.7+omo.1 (2026-04-19) — oh-my-opencode Integration Fork

This is a fork of superpowers v5.0.7 adapted for oh-my-opencode's agent ecosystem. All skills are renamed with `omo-` prefix and dispatch instructions updated to route through oh-my-opencode's `task()` and `call_omo_agent()` tools.

### Skill Renaming

All 14 skills renamed with `omo-` prefix to avoid namespace collisions:

| Original | Renamed |
|----------|---------|
| `brainstorming` | `omo-brainstorming` |
| `dispatching-parallel-agents` | `omo-dispatching-parallel-agents` |
| `executing-plans` | `omo-executing-plans` |
| `finishing-a-development-branch` | `omo-finishing-a-development-branch` |
| `receiving-code-review` | `omo-receiving-code-review` |
| `requesting-code-review` | `omo-requesting-code-review` |
| `subagent-driven-development` | `omo-subagent-driven-development` |
| `systematic-debugging` | `omo-systematic-debugging` |
| `test-driven-development` | `omo-test-driven-development` |
| `using-git-worktrees` | `omo-using-git-worktrees` |
| `using-superpowers` | `omo-using-superpowers` |
| `verification-before-completion` | `omo-verification-before-completion` |
| `writing-plans` | `omo-writing-plans` |
| `writing-skills` | `omo-writing-skills` |

All cross-references, frontmatter `name` fields, `GEMINI.md`, `hooks/session-start`, `agents/code-reviewer.md`, `.github/PULL_REQUEST_TEMPLATE.md`, `commands/`, `docs/`, `tests/`, and `README.md` updated accordingly.

### Agent Role Mapping to oh-my-opencode

Superpowers' subagent roles now dispatch to oh-my-opencode's specialized agents:

- **Implementer** → `task(category="quick|deep|ultrabrain|visual-engineering", load_skills=["omo-test-driven-development"])` — category-based model routing replaces manual model selection
- **Spec Compliance Reviewer** → `task(subagent_type="momus")` — uses Momus agent for spec verification with `[OKAY]`/`[REJECT]` verdicts
- **Code Quality Reviewer** → `task(subagent_type="oracle")` — uses Oracle agent for architecture and quality review
- **Codebase Search** → `task(subagent_type="explore", run_in_background=true)` — uses Explore agent for fast parallel grep
- **External Research** → `task(subagent_type="librarian", run_in_background=true)` — uses Librarian agent for docs/code search
- **Pre-planning Analysis** → `task(subagent_type="metis")` — uses Metis agent for intent classification and ambiguity detection
- **Plan Review** → `task(subagent_type="momus")` — uses Momus agent for plan executability verification

### Key File Changes

- `skills/omo-subagent-driven-development/SKILL.md` — 38 `task()` dispatch calls replacing generic `Task tool` instructions; added Category Selection Guide table
- `skills/omo-subagent-driven-development/implementer-prompt.md` — rewrote dispatch template to use `task(category=..., load_skills=["omo-test-driven-development"])` format with category selection guide
- `skills/omo-subagent-driven-development/spec-reviewer-prompt.md` — rewrote to dispatch via `subagent_type="momus"`, output format changed from ✅/❌ to `[OKAY]`/`[REJECT]`
- `skills/omo-subagent-driven-development/code-quality-reviewer-prompt.md` — rewrote to dispatch via `subagent_type="oracle"`
- `skills/omo-dispatching-parallel-agents/SKILL.md` — parallel dispatch now uses `run_in_background=true` with `background_output(task_id=...)` result collection
- `skills/omo-requesting-code-review/SKILL.md` — code review dispatches to Oracle agent instead of internal code-reviewer
- `skills/omo-writing-plans/SKILL.md` — execution handoff updated with OmO category-based task dispatch format and parallel wave instructions
- `skills/omo-brainstorming/SKILL.md` — implementation section now routes to Metis for pre-planning analysis before omo-writing-plans
- `agents/code-reviewer.md` — renamed to `omo-code-reviewer`, description updated to point to Oracle/Momus agents
- `hooks/session-start` — reads from `omo-using-superpowers/SKILL.md`
- `GEMINI.md` — references `omo-using-superpowers` paths
- `docs/` — all 99 skill path references and 12 namespace references updated to `omo-` prefix

### Discipline Preserved

All Superpowers discipline content is preserved verbatim: two-stage review (spec compliance → code quality), TDD Iron Law, 4-status reporting (DONE/DONE_WITH_CONCERNS/BLOCKED/NEEDS_CONTEXT), Red Flags tables, self-review checklists, systematic debugging phases, and verification-before-completion gates.
