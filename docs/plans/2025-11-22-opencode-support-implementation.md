...
> **For agentic workers:** REQUIRED SUB-SKILL: Use omo-executing-plans to implement this plan task-by-task.
 ...
 *
 * @param {string} skillName - Name like "omo-brainstorming" or "my-skill"
 * @param {string} superpowersDir - Path to superpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superpowersDir, personalDir) {
    // Strip omo- prefix if present
    const forceSuperpowers = skillName.startsWith('omo-');
    const actualSkillName = forceSuperpowers ? skillName.replace(/^omo-/, '') : skillName;

    // Try personal skills first (unless explicitly omo-)
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "omo-brainstorming" or "my-custom-skill")')
        }),
...
```javascript
      {
        ...
        execute: async () => {
          ...
          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'omo-';
...
**Below is the full content of your 'omo-using-superpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
```
use use_skill tool with skill_name: "omo-brainstorming"
```
...
Run: `.codex/superpowers-codex use-skill omo-brainstorming | head -20`
